...
from agents import create_code, pdf_checker_agent, code_enhancer
...
dic, config = define_variables(thread = 1, loop_limit = 10, data = data, data_info = data_info, name = "fix-vis", code = code, input = previous)
dic['revise'] = True
code_lead = code_enhancer(State)
for chunk in code_lead.stream(input = dic, config = config):
    pretty_print_messages(chunk)
