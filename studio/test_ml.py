...
from agents import create_output_plotly_team, test_single_agent, chain, machinelearning_agent
...
for i in range(48):
    ans = machinelearning_agent(State).invoke(input = dic, config =config)
    ideas = get_last_ai_message(ans['messages'])
    with open("sml_agent_ideas.txt", "a", encoding="utf-8") as f:
