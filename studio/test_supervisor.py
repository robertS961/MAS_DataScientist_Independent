...
from agents import create_output_plotly_team, test_single_agent, supervisor
...
for i in range(48):
    ans = supervisor(State).invoke(input = dic, config =config)
    print(f"This is the output \n{ans} \n")
    ideas = get_last_ai_message(ans['messages'])
    with open("research_agent_ideas.txt", "a", encoding="utf-8") as f:
