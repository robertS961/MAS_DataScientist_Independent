...
def ploty_enhancer_agent(state: State):
    prompt = (
        ...
        "Your task is to enhance the existing plotly code by improving the visualizations, adding interactivity, and ensuring that the graphs are clear and informative. Make sure the colors are not too similar as it is difficult to see! \n"
        ...
        f"Use this information also {state['data_info']}, make sure the cod eis prepared for NaN values, duplicate values, and missing values! \n"
