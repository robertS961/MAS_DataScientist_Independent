...
from agents import create_output_plotly_team, test_single_agent
...
for i in range(50):
    ans = test_single_agent(State).invoke(input = dic, config = config)
    ideas = get_last_ai_message(ans['messages'])

    with open("single_agent_ideas.txt", "a", encoding="utf-8") as f:
