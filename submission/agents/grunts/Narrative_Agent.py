...
def narrative_agent(state: State):
    ...
    prompt = (
        ...
        "If the graph doesn't have a narrative then write one, once again inside the correct HTML tag. The goal is to explain what the graph is doing, why its important, and the significance of the findings and why these fgindings are important \n"
        "Make sure each pharagraph flows into the next one just like an actual report \n"
        "Lastly add an introduction to the entire flow and a conclusion. Make it as academic and educational as possible\n"
