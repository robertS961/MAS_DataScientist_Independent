...
def ploty_agent(state: State):
    ...
    prompt = (
        ...
        "When displaying the graphs please keep the following format : Main title, PLENTY OF  space, Sub titles(if any), PLENTY OF space, then interactive buttons(keys,toggles,) make sure to give them space between them, then space, then graph \n"
        ...
        "Make sure the toggle buttons for the graph isn't on top of the key on the far right. THIS HAPPENS ALOT \n"
        "Make sure the buttond description isnt right under the button it is NOT READABLE. MOVE IT LOWER OR TO the side! \n"
