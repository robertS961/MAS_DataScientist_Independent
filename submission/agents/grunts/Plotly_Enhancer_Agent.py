...
def ploty_enhancer_agent(state: State):
    ...
    prompt = (
        ...
        "Make sure on the double graphs where they are displayed next to each other that there is plenty of space between. Also on single graphs or double there is occasionaly keys or right side bar/key is on top of the graph or intersecting with another graph. Give the graphs space and give they right side keys/bar enough space too. Plenty of scroll room to the right or left! \n"
        ...
        "The description of what the button does that goes away once the button is selected is normally right under the button. IT IS UNREADABLE, please remove or fix this! \n"
        "Make sure we keep the display of title, space, subtitle(if any), space, even more space,  then the graph, space, lastly interactive buttons or toggles. Do not put keys or buttons on top of titles, axis, right or left axis etc. \n"
